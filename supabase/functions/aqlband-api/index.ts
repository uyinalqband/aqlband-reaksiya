...
interface DuelRow {
  ...
  checkers_skin_id: string | null;
  created_at: string;
}

const CHECKERS_SKIN_REQUIRED_LEVEL = {
  classic: 1,
  royal: 5,
  emerald: 10,
  obsidian: 20,
...
type CheckersSkinId = keyof typeof CHECKERS_SKIN_REQUIRED_LEVEL;
const DEFAULT_CHECKERS_SKIN_ID: CheckersSkinId = 'classic';

function normalizeCheckersSkinId(value: unknown): CheckersSkinId {
  return typeof value === 'string' &&
    Object.prototype.hasOwnProperty.call(
      CHECKERS_SKIN_REQUIRED_LEVEL,
      value,
    )
    ? (value as CheckersSkinId)
...
async function getProgressionLevel(
  ...
  const { data, error } = await serviceClient.rpc('get_progression', {
    p_user_id: userId,
  });
  if (error) {
    throw new ApiError(
      'Daraja ma’lumotini yuklab bo‘lmadi.',
      500,
...
async function getUserCheckersSkin(
  serviceClient: SupabaseClient,
  userId: string,
): Promise<CheckersSkinId> {
  const { data, error } = await serviceClient
    .from('users')
    .select('selected_checkers_skin')
    .eq('id', userId)
    .maybeSingle();
  if (error || !data) {
    throw new ApiError(
      'Shashka skinini yuklab bo‘lmadi.',
      500,
      'checkers_skin_load_failed',
    );
  }
  return normalizeCheckersSkinId(data.selected_checkers_skin);
}

async function getCheckersAppearanceSnapshot(
  serviceClient: SupabaseClient,
  userId: string,
) {
  const [selectedSkinId, level] = await Promise.all([
    getUserCheckersSkin(serviceClient, userId),
    getProgressionLevel(serviceClient, userId),
  ]);
  const unlockedSkinIds = (
    Object.keys(CHECKERS_SKIN_REQUIRED_LEVEL) as CheckersSkinId[]
  ).filter(
    (skinId) =>
      level >= CHECKERS_SKIN_REQUIRED_LEVEL[skinId],
  );

  return { selectedSkinId, level, unlockedSkinIds };
}

async function lockCheckersDuelSkin(
  serviceClient: SupabaseClient,
  duel: DuelRow,
): Promise<DuelRow> {
  if (
    duel.game_id !== 'checkers' ||
    duel.checkers_skin_id ||
    !duel.guest_user_id
  ) {
    return duel;
  }

  const skinId = await getUserCheckersSkin(
    serviceClient,
    duel.guest_user_id,
  );
  const { data, error } = await serviceClient
    .from('duels')
    .update({ checkers_skin_id: skinId })
    .eq('id', duel.id)
    .is('checkers_skin_id', null)
    .select('*')
    .maybeSingle();

  if (error) {
    throw new ApiError(
      'O‘yin skinini saqlab bo‘lmadi.',
      500,
      'checkers_skin_lock_failed',
    );
  }
  if (data) return data as DuelRow;
  ...
  if (currentError || !current) {
    throw new ApiError(
      'O‘yin skinini yuklab bo‘lmadi.',
      500,
      'checkers_skin_load_failed',
    );
  }
  return current as DuelRow;
}
...
async function getDuelOrThrow(serviceClient: SupabaseClient, duelId: string): Promise<DuelRow> {
  ...
  if (
    ...
    return lockCheckersDuelSkin(
      serviceClient,
      await resolveCheckersTimeoutIfNeeded(
        serviceClient,
        rawDuel,
      ),
    );
...
async function handleAction(
  ...
  switch (action) {
    ...
    case 'checkers.appearance.get':
      return getCheckersAppearanceSnapshot(
        serviceClient,
        account.id,
      );

    case 'checkers.appearance.update': {
      const requestedSkinId = requireString(
        payload.skinId,
        'skinId',
        40,
      );
      if (
        ...
        throw new ApiError(
          ...
          400,
          'checkers_skin_not_found',
        );
      }

      const skinId = requestedSkinId as CheckersSkinId;
      const level = await getProgressionLevel(
        serviceClient,
        account.id,
      );
      const requiredLevel =
        CHECKERS_SKIN_REQUIRED_LEVEL[skinId];
      if (level < requiredLevel) {
        throw new ApiError(
          `Bu skin uchun ${requiredLevel}-daraja kerak.`,
          403,
          'checkers_skin_locked',
        );
      }
      ...
      if (error) {
        throw new ApiError(
          'Shashka skinini tanlab bo‘lmadi.',
          500,
          'checkers_skin_update_failed',
        );
      }
    ...
    case 'duel.invite': {
      ...
      const { data: target, error: targetError } = await serviceClient
        .from('users')
        .select('id,display_name,selected_checkers_skin')
        .eq('id', targetUserId)
        .maybeSingle();
    ...
    case 'duel.join': {
      ...
      duel = await lockCheckersDuelSkin(serviceClient, duel);
      return duelSnapshot(duel);
    }
    ...
    case 'duel.checkers_rematch': {
      ...
      const { data, error } = await serviceClient
        ...
        .insert({
          ...
          game_config: {},
          checkers_mode: 'friendly',
          checkers_skin_id: normalizeCheckersSkinId(
            opponent.selected_checkers_skin,
          ),
