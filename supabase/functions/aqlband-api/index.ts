...
async function performAiCheckersTurn(
  ...
  for (let step = 0; step < 12; step += 1) {
    ...
    // Captures and promotions are preferred; equal moves are varied so the
    // persona feels natural while every move remains server-valid.
    const ranked = moves
      .map((move) => {
        const applied = applyServerCheckersMove(board, move);
        const replyCount = serverCheckersLegalMoves(
          applied.board,
          'white',
          null,
        ).length;
        return {
          move,
          score:
            (move.captured !== null ? 100 : 0) +
            (applied.promoted ? 35 : 0) -
            replyCount * 0.3 +
            Math.random() * 4,
        };
      })
  ...
  switch (action) {
    ...
    case 'match.offer.inbox': {
      ...
      return { offer: {
        ...
        seekerName: String(user?.display_name ?? 'AqlBand'),
