...
interface DuelRow {
  ...
  game_id: 'reaction' | 'emoji-find' | 'number-memory' | 'stroop-test' | 'ascending-numbers' | 'odd-one-out' | 'pattern-memory' | 'go-no-go' | 'mental-math' | 'sequence-memory' | 'card-memory' | 'time-estimation' | 'peripheral-vision' | 'twenty-four' | 'dual-n-back' | 'fifteen-puzzle';
  round_count: number;
  survival: boolean;
...
function sanitizeAttempt(value: unknown): GameAttemptInput {
  ...
  const durationMax = gameId === 'reaction' ? 10_000 : 300_000;
  const valid =
