...
interface DuelRow {
  ...
  game_id: 'reaction' | 'emoji-find' | 'number-memory' | 'stroop-test' | 'ascending-numbers' | 'odd-one-out' | 'pattern-memory' | 'go-no-go' | 'mental-math' | 'sequence-memory' | 'card-memory' | 'time-estimation' | 'peripheral-vision' | 'twenty-four' | 'dual-n-back' | 'fifteen-puzzle' | 'sudoku' | 'tic-tac-toe';
  round_count: number;
  survival: boolean;
...
const DUEL_GAME_IDS = [
  ...
  'ascending-numbers', 'odd-one-out', 'pattern-memory', 'go-no-go',
  'mental-math', 'sequence-memory', 'card-memory', 'time-estimation',
  'peripheral-vision', 'twenty-four', 'dual-n-back', 'fifteen-puzzle', 'sudoku', 'tic-tac-toe',
] as const;
