  ...
  switch (action) {
    ...
    case 'match.offer.inbox': {
      ...
      const { data } = await serviceClient.from('checkers_match_offers')
        .select('id,seeker_user_id,seeker_rating,expires_at,created_at,users!checkers_match_offers_seeker_user_id_fkey(display_name)')
        .eq('target_user_id', account.id).eq('status', 'pending')
        .gt('expires_at', new Date().toISOString())
        .order('created_at', { ascending: false }).limit(1).maybeSingle();
      if (!data) return { offer: null };
      const joined = data as Record<string, unknown>;
      ...
      return { offer: {
        ...
        expiresAt: new Date(String(joined.expires_at)).getTime(),
      }};
    }
    ...
    case 'match.offer.respond': {
      ...
      if (error || !data) throw new ApiError('Taklif eskirgan.', 409, 'offer_expired');
