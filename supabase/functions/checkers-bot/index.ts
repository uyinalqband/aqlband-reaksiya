...
async function deliverOutbox(token: string, db: SupabaseClient) {
  // A function instance may stop after claiming a row. Recover such rows so a
  // transient crash never loses a notification permanently.
  ...
  for (const recipient of recipients ?? []) {
    ...
    try {
      ...
      if (/blocked|chat not found|deactivated/i.test(message)) {
        ...
        // Preserve partial failures for the normal retry worker. The dedupe key
        // prevents repeated cron calls from creating duplicate messages.
        await db.from('bot_notification_outbox').upsert({
          ...
          dedupe_key: `${range.key}:${recipient.telegram_id}`,
          last_error: message.slice(0, 500),
