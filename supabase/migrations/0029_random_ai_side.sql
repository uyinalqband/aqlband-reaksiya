-- Checkers Online: AI va foydalanuvchi rangini har yangi o‘yinda tasodifiy tanlash.
-- 0025_consent_ai_rating.sql dan keyin bir marta SQL Editor orqali ishga tushiring.
...
begin
  ...
  return jsonb_build_object(
    ...
    'opponentName',bot.display_name,'opponentType','ai',
...
begin
  ...
  if not found or d.opponent_type<>'ai' or d.checkers_rating_processed_at is null then
    return d;
  end if;

  human_role := case
    when coalesce(d.game_config->>'aiRole','guest')='host' then 'guest'
    else 'host'
  end;
  human_id := case when human_role='host' then d.host_user_id else d.guest_user_id end;
  bot_id := case when human_role='host' then d.guest_user_id else d.host_user_id end;
  human_before := case when human_role='host'
    then coalesce(d.checkers_host_rating_before,1200)
    else coalesce(d.checkers_guest_rating_before,1200)
  end;
  original_delta := case when human_role='host'
    then coalesce(d.checkers_host_rating_delta,0)
    else coalesce(d.checkers_guest_rating_delta,0)
  end;
  bot_before := case when human_role='host'
    then coalesce(d.checkers_guest_rating_before,1200)
    else coalesce(d.checkers_host_rating_before,1200)
  end;

  select count(*) into today_ai_games
  from public.checkers_rating_events e
  join public.duels x on x.id=e.duel_id
  where e.user_id=human_id and x.opponent_type='ai'
  ...
  where user_id=human_id;
  update public.checkers_ratings set
    rating=bot_before,
    peak_rating=greatest(peak_rating,bot_before),
    games=greatest(0,games-1),
    wins=greatest(0,wins-case when d.checkers_winner<>human_role and d.checkers_winner<>'draw' then 1 else 0 end),
    draws=greatest(0,draws-case when d.checkers_winner='draw' then 1 else 0 end),
    losses=greatest(0,losses-case when d.checkers_winner=human_role then 1 else 0 end)
  where user_id=bot_id;

  update public.checkers_rating_events set
    rating_after=greatest(100,human_before+limited_delta),
    rating_delta=limited_delta
  where duel_id=d.id and user_id=human_id;
  delete from public.checkers_rating_events
  where duel_id=d.id and user_id=bot_id;

  update public.duels set
    checkers_host_rating_after=case when human_role='host'
      then greatest(100,human_before+limited_delta) else bot_before end,
    checkers_host_rating_delta=case when human_role='host' then limited_delta else 0 end,
    checkers_guest_rating_after=case when human_role='guest'
      then greatest(100,human_before+limited_delta) else bot_before end,
    checkers_guest_rating_delta=case when human_role='guest' then limited_delta else 0 end
