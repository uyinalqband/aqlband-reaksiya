...
create or replace function public.checkers_online_json_int(
  p_meta jsonb,
  p_key text,
...
create or replace function public.checkers_online_raw_xp(
  p_game_id text,
  p_value integer,
...
begin
  case p_game_id
    ...
    when 'emoji-find' then
      errors_count := greatest(0, least(100, public.checkers_online_json_int(p_meta, 'errors', 0)));
      result := case
    ...
    when 'stroop-test' then
      correct_count := greatest(0, least(8, public.checkers_online_json_int(p_meta, 'correct', 0)));
      errors_count := greatest(0, least(8, public.checkers_online_json_int(p_meta, 'errors', 0)));
      timeouts_count := greatest(0, least(8, public.checkers_online_json_int(p_meta, 'timeouts', 0)));
      average_ms := greatest(0, public.checkers_online_json_int(p_meta, 'averageMs', 0));
      speed_bonus := case
...
create or replace function public.checkers_online_xp_for_level(p_level integer)
returns bigint
language sql
immutable
as $$
...
create or replace function public.checkers_online_level_from_xp(p_total_xp bigint)
returns integer
language sql
immutable
as $$
...
begin
  ...
  raw_xp := public.checkers_online_raw_xp(p_game_id, p_value, coalesce(p_meta, '{}'::jsonb));
...
create or replace function public.checkers_online_award_xp_trigger()
returns trigger
language plpgsql
security definer
set search_path = public
...
for each row execute function public.checkers_online_award_xp_trigger();
...
as $$
  with me as (
    select
      ...
      u.total_xp,
      u.created_at,
      public.checkers_online_level_from_xp(u.total_xp) as level
    from public.users u
  ...
  select
    me.total_xp,
    me.level,
    public.checkers_online_xp_for_level(me.level) as current_level_xp,
    public.checkers_online_xp_for_level(me.level + 1) as next_level_xp,
    coalesce((
...
as $$
  ...
  select
    ...
    r.username,
    r.total_xp,
    public.checkers_online_level_from_xp(r.total_xp),
    r.rank
  from ranked r
  order by r.rank
...
revoke all on function public.checkers_online_json_int(jsonb, text, integer) from public, anon, authenticated;
revoke all on function public.checkers_online_raw_xp(text, integer, jsonb) from public, anon, authenticated;
revoke all on function public.checkers_online_xp_for_level(integer) from public, anon, authenticated;
revoke all on function public.checkers_online_level_from_xp(bigint) from public, anon, authenticated;
...
revoke all on function public.checkers_online_award_xp_trigger() from public, anon, authenticated;
