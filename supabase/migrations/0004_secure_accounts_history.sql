...
set
  ...
  display_name = coalesce(nullif(btrim(display_name), ''), nullif(btrim(first_name), ''), 'AI'),
...
select
  'telegram',
  l.telegram_id::text,
  coalesce(nullif(max(l.first_name), ''), 'AI'),
  max(l.username),
  l.telegram_id,
  coalesce(nullif(max(l.first_name), ''), 'AI')
...
select distinct
  ...
  coalesce(nullif(d.host_name, ''), 'AI'),
  d.host_telegram_id,
  coalesce(nullif(d.host_name, ''), 'AI')
from public.duels d
left join public.users u
...
select distinct
  ...
  coalesce(nullif(d.guest_name, ''), 'AI'),
  d.guest_telegram_id,
  coalesce(nullif(d.guest_name, ''), 'AI')
from public.duels d
left join public.users u
