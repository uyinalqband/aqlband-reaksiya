-- ============================================================
-- Expand database constraints for the modular game catalog.
-- Safe to re-run after migrations 0001-0009.
-- ============================================================
...
alter table public.game_attempts add constraint game_attempts_game_check check (
  game_id in ('reaction', 'emoji-find', 'number-memory', 'stroop-test', 'ascending-numbers', 'odd-one-out', 'pattern-memory', 'go-no-go', 'mental-math', 'sequence-memory', 'card-memory', 'time-estimation', 'peripheral-vision', 'twenty-four', 'dual-n-back', 'fifteen-puzzle', 'duel-reaction')
);
...
alter table public.xp_events alter column rule_version set default 3;
...
declare
  base_xp integer := 0;
  result integer := 0;
  completed_rounds integer := greatest(1, least(100, public.checkers_online_json_int(p_meta, 'rounds', 1)));
  correct_count integer := greatest(0, least(100, public.checkers_online_json_int(p_meta, 'correct', completed_rounds)));
...
begin
  ...
  if p_game_id in ('reaction', 'emoji-find', 'number-memory', 'stroop-test', 'ascending-numbers', 'odd-one-out', 'pattern-memory', 'go-no-go', 'mental-math', 'sequence-memory', 'card-memory', 'time-estimation', 'peripheral-vision', 'twenty-four', 'dual-n-back', 'fifteen-puzzle') and p_metric <> 'duration_ms' then
    raw_xp := 0;
  else
  ...
  insert into public.xp_events (
    ...
    multiplier, daily_bonus, variety_bonus, pb_bonus, final_xp, 3, p_played_at
  )
