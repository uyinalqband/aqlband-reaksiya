-- ============================================================
-- Checkers Online V2.6 direct-start and 60-second turn fix.
-- Run after 0018_direct_game_start.sql. Safe to re-run.
-- ============================================================

-- Old ready/countdown rows are promoted immediately so the board can be used
...
set
  ...
  checkers_turn_deadline_at = case
    when game_id = 'checkers' then now() + interval '60 seconds'
    else checkers_turn_deadline_at
  end
where game_id in ('checkers', 'tic-tac-toe')
  and status in ('ready_check', 'countdown');

-- Normalize any active Checkers match created by an older release.
update public.duels
set
  game_start_at = case
    when game_start_at is null or game_start_at > now()
      then now()
    else game_start_at
  end,
  checkers_turn_deadline_at = now() + interval '60 seconds'
where game_id = 'checkers'
  and status = 'playing'
  and checkers_winner is null;

-- Rated matchmaking now creates a fully active match. Every turn, including
-- White's first move, receives exactly 60 seconds.
