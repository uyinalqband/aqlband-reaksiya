...
/** Fixed fast-play configuration used by all solo mini-games in Checkers Online V2.6. */
...
const PROGRESSIVE_SURVIVAL_SESSION: GameSessionConfig = {
  rounds: 'survival',
  difficulty: 'progressive',
  memorySize: 4,
};

const SOLO_GAME_SESSIONS: Partial<Record<SoloGameId, GameSessionConfig>> = {
  'emoji-find': {
    rounds: 5,
    difficulty: 'progressive',
  },
  'number-memory': PROGRESSIVE_SURVIVAL_SESSION,
  'stroop-test': PROGRESSIVE_SURVIVAL_SESSION,
  'odd-one-out': PROGRESSIVE_SURVIVAL_SESSION,
  'pattern-memory': PROGRESSIVE_SURVIVAL_SESSION,
  'mental-math': PROGRESSIVE_SURVIVAL_SESSION,
  'sequence-memory': PROGRESSIVE_SURVIVAL_SESSION,
};

export function getSoloGameSession(gameId: SoloGameId): GameSessionConfig {
  return { ...(SOLO_GAME_SESSIONS[gameId] ?? MAX_SOLO_SESSION) };
}
...
export function difficultyIndex(difficulty: Difficulty, round: number): number {
  ...
  return Math.min(3, Math.max(0, round - 1));
}

export function progressiveExtraRound(config: GameSessionConfig, round: number): number {
  return config.difficulty === 'progressive' ? Math.max(0, round - 4) : 0;
}
...
export const GAME_SETUP_COPY: Record<string,{title:string;emoji:string;subtitle:string;instructions:string[]}> = {
 reaction:{title:'Tezkor signal',emoji:'⚡',subtitle:'Signalni kuting.',instructions:['Signalni kuting','Yashilda bosing','O‘rtacha vaqt hisoblanadi']},
