  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        const incomingFriends = friends.filter((item) => item.status === 'pending' && !item.isOutgoing);
        const activeIds = new Set([
          ...invites.map((invite) => `duel-${invite.duel.id}`),
          ...incomingFriends.map((entry) => `friend-${entry.friendshipId}`),
        ]);

        for (const item of useNotificationStore.getState().items) {
          if ((item.id.startsWith('duel-') || item.id.startsWith('friend-')) && !activeIds.has(item.id)) {
        ...
        for (const entry of incomingFriends) {
          if (stopped) break;
          await upsert({
