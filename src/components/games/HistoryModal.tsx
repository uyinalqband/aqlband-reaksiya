...
const META_LABEL_KEYS: Record<string, string> = {
  ...
  opponent: 'history.meta.opponent',
  outcome: 'history.meta.outcome',
  opponentTimeMs: 'history.meta.opponentTimeMs',
  captured: 'history.meta.captured',
  opponentCaptured: 'history.meta.opponentCaptured',
  promotions: 'history.meta.promotions',
  durationMs: 'history.meta.durationMs',
  color: 'history.meta.color',
  resultReason: 'history.meta.resultReason',
};
...
export function HistoryModal({ onClose }: { onClose: () => void }) {
  ...
  function formatValue(key: string, value: unknown): string {
    ...
    if (
      (key === 'mode' || key === 'outcome' || key === 'color') &&
      typeof value === 'string'
    ) {
      return t(`history.values.${value}`, { defaultValue: value });
    }
    if (key === 'resultReason' && typeof value === 'string') {
      return t(`checkers.reasons.${value}`, { defaultValue: value });
    }
    if (
      (key === 'durationMs' ||
        key === 'opponentTimeMs' ||
          ...
          {attempts.length === 0 ? (
            ...
            attempts.map((attempt) => {
              ...
              const metaEntries = Object.entries(attempt.meta ?? {}).filter(
                ([key, value]) =>
                  !hiddenMetaKeys.has(key) &&
                  value !== undefined &&
                  value !== null &&
                  value !== '',
              );
              const outcome =
                typeof attempt.meta?.outcome === 'string'
