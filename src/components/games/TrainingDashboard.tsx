...
const SKILLS = [
  ['speed', 'training.skills.speed', '⚡'],
  ['memory', 'training.skills.memory', '🧠'],
  ['attention', 'training.skills.attention', '👁'],
  ['logic', 'training.skills.logic', '🧩'],
] as const;
    ...
    <>
      <section className="premium-border mt-5 rounded-3xl bg-gradient-to-br from-violet-500/15 to-ink-800/90 p-4 shadow-card">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[.18em] text-violet-300">{t('training.today')}</p>
            <h2 className="mt-1 font-display text-xl font-extrabold">{t('training.completed', { count: progress })}</h2>
            <p className="mt-1 text-xs text-mist-500">{t('training.subtitle')}</p>
          </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          {games.map((game, index) => (
            <button key={game.id} type="button" onClick={() => navigate(game.route, { state: { adaptive: true } })}
              ...
              <span className="mt-1 block text-[10px] font-extrabold text-mist-200">{t('training.exercise', { number: index + 1 })}</span>
            </button>
          ))}
        </div>
      </section>
      ...
      <button type="button" onClick={() => navigate(recommendation.route, { state: { adaptive: true } })}
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-[10px] font-black uppercase tracking-wider text-gold-300">{t('training.recommended')}</span>
          <span className="block font-display text-sm font-extrabold">{t('training.strengthen')}</span>
        </span>
      ...
      <section className="mt-3 rounded-3xl border border-white/10 bg-ink-800/70 p-4">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-sm font-extrabold">{t('training.skillMap')}</h2>
          <span className="text-[10px] text-mist-500">{t('training.last14Days')}</span>
        </div>
        <div className="mt-3 space-y-2.5">
          {SKILLS.map(([id, label, emoji]) => (
            <div key={id} className="grid grid-cols-[5rem_1fr_2rem] items-center gap-2">
              <span className="text-[11px] font-bold text-mist-300">{emoji} {t(label)}</span>
      ...
      <section className="mt-3 grid grid-cols-2 gap-3">
        <div className="rounded-3xl border border-cyan-300/15 bg-cyan-500/10 p-4">
          <p className="text-[10px] font-black uppercase tracking-wider text-cyan-300">{t('training.dailyMission')}</p>
          <p className="mt-2 text-sm font-extrabold">{t('training.threeSkills')}</p>
          <p className="mt-1 text-xs text-mist-500">{t('training.completedGames', { count: progress })}</p>
        </div>
        <div className="rounded-3xl border border-amber-300/15 bg-amber-500/10 p-4">
          <p className="text-[10px] font-black uppercase tracking-wider text-amber-300">{t('training.weeklyChallenge')}</p>
          <p className="mt-2 text-sm font-extrabold">{t('training.fiveActiveDays')}</p>
          <p className="mt-1 text-xs text-mist-500">{t('training.weekProgress', { days: Math.min(5, activeDays), count: weekAttempts.length })}</p>
        </div>
      </section>
    </>
  );
