...
const SOLO_MS_GAMES = GAMES.filter((game) => !game.friendOnly && game.metric === 'duration_ms').map((game) => ({
  id: game.id,
  emoji: game.emoji,
...
export function GameBestLeaderboardModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[85] flex items-end justify-center bg-black/65 px-3 pt-3 backdrop-blur-sm"
      onClick={onClose}
      style={{
    ...
    >
      <Card
        padded={false}
        className="flex max-h-[84dvh] w-full max-w-md flex-col overflow-hidden"
      ...
      >
        ...
        <div className="shrink-0 border-t border-ink-600/70 bg-ink-800/95 p-4">
          ...
          {currentUser ? (
            ...
            <p className="mt-2 rounded-xl border border-ink-600 bg-ink-900/60 px-3 py-3 text-sm text-mist-400">
              {t('home.notRankedYet')}
            </p>
          )}
        </div>
