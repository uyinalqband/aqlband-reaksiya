...
/**
 * Runs once on boot, inside the Router. If the app was opened via a duel
 * invite link (`startapp=duel_<id>`), redirects straight into DuelScreen as
 * the guest. Must live inside <BrowserRouter> to use useNavigate.
 */
...
export default function App() {
  ...
  const hydrateRank = useOnlineStore((s) => s.hydrateLastKnownRank);
  ...
  useEffect(() => {
    ...
    void Promise.all([hydrateStats(), hydrateSettings(detectedLanguage), hydrateRank()]).finally(() => {
  ...
  }, [hydrateStats, hydrateSettings, hydrateRank]);
