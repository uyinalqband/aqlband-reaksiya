...
const STORAGE_KEY = 'checkers_online_tezkor_signal_settings_v1';
...
export const useSettingsStore = create<SettingsState>((set, get) => ({
    ...
    try {
      ...
      if (raw) {
        ...
        const merged: PersistedSettings = {
          ...
          effectsVolume:
            typeof saved.effectsVolume === 'number'
              ? Math.max(0, Math.min(1, saved.effectsVolume))
  ...
  setEffectsVolume: async (volume) => {
    const effectsVolume = Math.max(0, Math.min(1, volume));
    set({ effectsVolume });
    await persist({
      language: get().language,
      soundEnabled: get().soundEnabled,
