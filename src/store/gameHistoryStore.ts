...
export type GameId =
  | 'checkers'
  | 'tic-tac-toe'
  | 'reaction' | 'emoji-find' | 'number-memory' | 'stroop-test' | 'duel-reaction'
...
const LEGACY_V2_MANIFEST_KEY = 'checkers_online_game_history_v2_manifest';
const LEGACY_V2_CHUNK_PREFIX = 'checkers_online_game_history_v2_chunk_';
const LEGACY_REACTION_KEY = 'checkers_online_tezkor_signal_attempts_v1';
const LEGACY_GAMES_KEY = 'checkers_online_game_stats_v1';
const MAX_HISTORY = 30;
const MAX_PER_GAME = 30;
...
function manifestKey(scopeKey: string): string {
  return `checkers_online_game_history_v3_${normalizeScope(scopeKey)}_manifest`;
}

function chunkPrefix(scopeKey: string): string {
  return `checkers_online_game_history_v3_${normalizeScope(scopeKey)}_chunk_`;
}
...
function isGameId(value: unknown): value is GameId {
  return typeof value === 'string' && [
    'checkers','tic-tac-toe','reaction','emoji-find','number-memory','stroop-test','duel-reaction',
    'ascending-numbers','odd-one-out','pattern-memory','go-no-go',
    'mental-math','sequence-memory','card-memory','time-estimation',
    'peripheral-vision','twenty-four','dual-n-back','fifteen-puzzle','sudoku',
  ].includes(value);
...
export const useGameHistoryStore = create<GameHistoryState>((set, get) => ({
  ...
  hydrate: async (rawScopeKey) => {
    const resolvedScopeKey = rawScopeKey ?? (await getLocalIdentityScope());
    const scopeKey = normalizeScope(resolvedScopeKey);
...
/** Best value from the unified millisecond result system. Legacy score/count rows are ignored. */
export function getBestDurationValue(attempts: GameAttempt[], gameId: GameId): number | null {
  const values = attempts
    .filter((attempt) => attempt.gameId === gameId && attempt.metric === 'duration_ms')
...
export function computeReactionStats(attempts: GameAttempt[]): ReactionStatsSnapshot {
  const reactionValues = attempts
    .filter((attempt) => attempt.gameId === 'reaction' && attempt.metric === 'duration_ms')
    .map((attempt) => attempt.value);
...
/** Shared statistics for all four solo games, now measured in milliseconds. */
export function computeUnifiedMsStats(attempts: GameAttempt[]): ReactionStatsSnapshot {
  const values = attempts
    .filter((attempt) => attempt.gameId !== 'duel-reaction' && attempt.metric === 'duration_ms')
    .map((attempt) => attempt.value);
