  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        let account = await ensureUser(controller.signal);
        if (cancelled) return;

        const localAvatar = useAvatarStore.getState().avatar;
        if (account.avatar !== localAvatar) {
          account = await updateAvatar(localAvatar).catch(() => account);
        }
