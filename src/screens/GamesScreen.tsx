...
import { TrainingDashboard } from '@/components/games/TrainingDashboard';
...
export function GamesScreen() {
  ...
  const bestFor = (game: GameDefinition) => {
    const values = attempts.filter((attempt) => attempt.gameId === game.id).map((attempt) => attempt.value);
    if (values.length === 0) return null;
    return game.metric === 'duration_ms' ? Math.min(...values) : Math.max(...values);
  };
  ...
  return (
    <Screen className="pb-28">
      ...
      <TrainingDashboard attempts={attempts} />

      <button
        type="button"
        onClick={() => navigate('/games/checkers-puzzles')}
        className="premium-border mt-4 flex w-full items-center gap-4 rounded-3xl bg-gradient-to-r from-emerald-500/15 to-ink-800/90 p-4 text-left shadow-card active:scale-[.99]"
      >
        <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-emerald-300/20 bg-emerald-400/10 text-3xl">♟️</span>
        <span className="min-w-0 flex-1">
          <span className="block text-[10px] font-black uppercase tracking-[.18em] text-emerald-300">
            {t('gameHub.dailyPuzzle')}
          </span>
          <span className="mt-1 block font-display text-base font-extrabold">{t('games.checkersPuzzle.title')}</span>
          <span className="mt-1 block text-xs text-mist-500">{t('gameHub.dailyPuzzleDescription')}</span>
        </span>
      ...
      <div className="mt-5 flex items-end justify-between gap-3">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[.18em] text-violet-300">{t('gameHub.libraryEyebrow')}</p>
          <h2 className="font-display text-xl font-extrabold">{t('gameHub.allExercises')}</h2>
        </div>
        <span className="text-xs text-mist-600">{games.length}</span>
      </div>
          ...
          >
            ...
            <span className="absolute bottom-3.5 left-4 text-[9px] font-black uppercase tracking-wider text-violet-300">
              {game.friendOnly
                ...
                : best === null
                  ? `1–2 MIN · +XP`
                  : `${t('gameHub.best')}: ${(best / 1000).toFixed(1)}s`}
            </span>
          </motion.button>
          );
        })}
