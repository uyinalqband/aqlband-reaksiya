...
export function SettingsScreen() {
  ...
  const handleSoundToggle = async () => {
    ...
    await toggleSound();
    sfx.configure(nextEnabled, effectsVolume);
    checkersMusic.configure(
      musicEnabled,
      musicVolume,
      nextEnabled,
    ...
    if (nextEnabled) sfx.notification();
  };
            ...
            <div className="px-4 py-4">
              ...
              <input
                ...
                onChange={(event: ChangeEvent<HTMLInputElement>) => {
                  const volume = Number(event.target.value) / 100;
                  sfx.unlock();
                  sfx.configure(soundEnabled, volume);
                  checkersMusic.configure(
                    musicEnabled,
                    musicVolume,
                    soundEnabled,
                  ...
                  void setEffectsVolume(volume);
                }}
                onPointerUp={() => sfx.notification()}
                className="mt-4 w-full accent-emerald-400 disabled:opacity-40"
              ...
              <button
                ...
                disabled={!soundEnabled}
                onClick={() => {
                  sfx.configure(soundEnabled, effectsVolume);
                  sfx.notification();
                }}
                className="mt-3 rounded-xl border border-white/10 bg-black/15 px-3 py-2 text-xs font-bold text-mist-300 disabled:opacity-40"
              >
