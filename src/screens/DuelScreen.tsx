...
const DUEL_SESSION_KEY = 'checkers_online_active_duel_v2';
...
export function DuelScreen() {
  ...
  const { duel, loading, error, markReady, cancel } = useDuel(
    ...
    telegramUser?.firstName ?? 'O‘yinchi',
  );
  ...
  useLayoutEffect(() => {
    if (
      ...
      !state ||
      launchedRef.current ||
      !['ready_check', 'countdown', 'playing'].includes(duel.status)
    ) {
      return;
    }
  ...
  return (
    <Screen>
      ...
      {loading ? (
        ...
        <div className="min-h-[64vh]" aria-hidden="true" />
