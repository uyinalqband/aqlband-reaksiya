...
import { CheckersPieceSkinPreview } from '@/components/checkers/CheckersPieceSkinPreview';
...
import {
  updateCheckersPieceSkin,
  updateCheckersSkin,
} from '@/services/checkersSkinService';
...
import {
  CHECKERS_PIECE_SKINS,
  isCheckersPieceSkinUnlocked,
  normalizeCheckersPieceSkinId,
  type CheckersPieceSkin,
...
export function ProfileScreen() {
  ...
  const displayName = account?.displayName ?? user?.firstName ?? 'AI';
  ...
  const selectedPieceSkinId = normalizeCheckersPieceSkinId(
    account?.selectedCheckersPieceSkin,
  );
  ...
  const selectBoardSkin = async (skin: CheckersSkin) => {
    if (
      skinSaving ||
  ...
  const selectPieceSkin = async (skin: CheckersPieceSkin) => {
    if (
      skinSaving ||
      selectedPieceSkinId === skin.id ||
      !isCheckersPieceSkinUnlocked(skin, level.level)
    ) {
      return;
    }

    setSkinSaving(`piece:${skin.id}`);
    setSkinError(null);
    try {
      const updated = await updateCheckersPieceSkin(skin.id);
      await setAccount(updated);
    } catch (error) {
      setSkinError(
        error instanceof Error ? error.message : t('errors.generic'),
  ...
  return (
    <Screen className="pb-28">
      ...
      <div className="no-scrollbar mt-4 grid grid-cols-3 rounded-2xl border border-white/10 bg-ink-900/70 p-1">
        {(['overview', 'friends', 'appearance'] as const).map((tab) => (
          <button
            key={tab}
            type="button"
            onClick={() => setProfileTab(tab)}
            className={`min-h-11 rounded-xl px-2 text-[11px] font-extrabold transition ${
              profileTab === tab
                ? 'bg-violet-500/25 text-violet-100 shadow-glow'
                : 'text-mist-500'
            }`}
          >
            {tab === 'overview' ? '👤' : tab === 'friends' ? '👥' : '🎨'}{' '}
            {t(`profile.tabs.${tab}`)}
          </button>
        ))}
      </div>
      ...
      <button
        ...
        className="mt-4 flex w-full items-center gap-3 rounded-3xl border border-gold-300/20 bg-gold-500/10 p-4 text-left active:scale-[.99]"
      >
        <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gold-400/10 text-2xl">🏅</span>
        <span className="min-w-0 flex-1">
          <span className="block font-display text-sm font-extrabold text-mist-100">Yutuqlar</span>
          <span className="mt-0.5 block text-[11px] text-mist-500">Badge, seriya va maxsus maqsadlar</span>
        </span>
      ...
      {user?.id ? (
        <button
          ...
          className="mt-3 flex w-full items-center gap-3 rounded-3xl border border-emerald-300/20 bg-emerald-500/10 p-4 text-left active:scale-[.99]"
        >
          <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-400/10 text-2xl">
            📨
          </span>
          <span className="min-w-0 flex-1">
            <span className="block font-display text-sm font-extrabold text-mist-100">
      ...
      <section className="mt-5">
        ...
        <div className="mt-4 grid grid-cols-2 rounded-2xl border border-white/10 bg-ink-900/65 p-1">
          <button
            type="button"
            onClick={() => setSkinStep('board')}
            className={`rounded-xl px-3 py-2.5 text-xs font-extrabold transition ${
              skinStep === 'board'
                ? 'bg-violet-500/25 text-violet-200 shadow-glow'
                : 'text-mist-500'
            }`}
          >
            1. {t('skins.steps.board')}
          </button>
          <button
            type="button"
            onClick={() => setSkinStep('pieces')}
            className={`rounded-xl px-3 py-2.5 text-xs font-extrabold transition ${
              skinStep === 'pieces'
                ? 'bg-violet-500/25 text-violet-200 shadow-glow'
                : 'text-mist-500'
        ...
        {skinStep === 'board' ? (
            ...
            {CHECKERS_SKINS.map((skin) => {
              ...
              return (
                <button
                  ...
                  onClick={() => void selectBoardSkin(skin)}
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            {CHECKERS_PIECE_SKINS.map((skin) => {
              const selected = selectedPieceSkinId === skin.id;
              const unlocked = isCheckersPieceSkinUnlocked(
                skin,
                level.level,
              );
              const saving = skinSaving === `piece:${skin.id}`;
              ...
              return (
                <button
                  ...
                  type="button"
                  disabled={!unlocked || Boolean(skinSaving)}
                  onClick={() => void selectPieceSkin(skin)}
                  className={`min-w-0 rounded-[1.7rem] border p-2.5 text-left transition active:scale-[.98] ${
                    selected
                      ? 'border-violet-300/70 bg-violet-500/12 shadow-glow'
                      : unlocked
                ...
                >
                  <CheckersPieceSkinPreview skinId={skin.id} />
                  <span className="mt-2.5 flex items-center justify-between gap-1.5">
                    <span className="min-w-0 truncate font-display text-sm font-extrabold text-mist-100">
                      {skin.emoji} {t(skin.nameKey)}
                    </span>
