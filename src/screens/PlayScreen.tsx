...
import { computeReactionStats, useGameHistoryStore } from '@/store/gameHistoryStore';
...
export function PlayScreen() {
  ...
  const snapshot = useMemo(() => computeReactionStats(attempts), [attempts]);
  const startedRef = useRef(false);
  const savedResultRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    if (phase !== 'result' || resultMs === null || savedResultRef.current === resultMs) return;
    savedResultRef.current = resultMs;

    const previousBest = snapshot.best;
    void addAttempt({
      gameId: 'reaction',
      value: Math.round(resultMs),
      metric: 'duration_ms',
      meta: { mode: 'solo' },
    }).then((attempt) => {
      const isNewBest = previousBest === null || resultMs < previousBest;
      navigate('/result', {
        state: { timeMs: resultMs, isNewBest, attemptId: attempt.id },
        replace: true,
      });
    });
  }, [phase, resultMs, snapshot.best, addAttempt, navigate]);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-8">
        ...
        {!isRetryable && <p className="max-w-[15rem] text-center text-sm text-mist-500">{t('game.prepareDesc')}</p>}
