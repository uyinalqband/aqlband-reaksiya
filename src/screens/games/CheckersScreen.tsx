...
export function CheckersScreen() {
  ...
  const skin = getCheckersSkin(duel?.checkers_skin_id);
  ...
  return (
    <Screen className="pb-8">
      ...
      <div
        className="premium-border mx-auto aspect-square w-[93%] max-w-[25rem] overflow-hidden rounded-[1.7rem] border p-2 shadow-[0_28px_65px_-28px_rgba(0,0,0,.98)]"
        style={{
            ...
            if (!dark) {
              return (
                <div
                  ...
                  style={{ background: skin.board.lightSquare }}
                />
              );
            }
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: skin.board.darkSquare,
                }}
              >
                {targetMove ? (
                  <span
...
function CheckersPieceView({
  ...
  const palette = white ? skin.pieces.white : skin.pieces.black;

  return (
    <span
      className={`pointer-events-none relative flex aspect-square w-[78%] shrink-0 items-center justify-center overflow-hidden rounded-full border-[2px] transition-transform duration-100 ${
        movable
          ? 'ring-2 ring-emerald-300/55 ring-offset-1'
          : ''
      } ${selected ? 'scale-110' : ''}`}
      style={{
        ...
        '--tw-ring-offset-color': skin.board.ringOffset,
      } as CSSProperties}
    >
      <span
        className="absolute inset-[15%] rounded-full border-2"
        style={{
          borderColor: palette.innerBorderColor,
          boxShadow: palette.innerShadow,
        }}
      />
      <span
        className="absolute inset-[27%] rounded-full border"
        style={{ borderColor: palette.detailBorderColor }}
      />
      {king ? (
        <svg
          viewBox="0 0 24 24"
          aria-hidden="true"
          className="relative z-10 h-[42%] w-[42%] drop-shadow-[0_3px_5px_rgba(0,0,0,.55)]"
          style={{ color: palette.kingColor }}
        >
          <path
            fill="currentColor"
            d="M4.2 18.4h15.6l1-8.3-4.3 3-2.2-6.1L12 11.2 9.7 7l-2.2 6.1-4.3-3 1 8.3Zm.8 1.5h14v1.6H5v-1.6Z"
          />
          <circle cx="3.2" cy="8.7" r="1.55" fill="currentColor" />
          <circle cx="12" cy="5.3" r="1.55" fill="currentColor" />
          <circle cx="20.8" cy="8.7" r="1.55" fill="currentColor" />
        </svg>
      ) : null}
    </span>
  );
...
function PlayerPanel({
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span
              ...
              style={{
                background: skin.pieces[side].background,
                borderColor: skin.pieces[side].borderColor,
              }}
            />
          </div>
      ...
      <CapturedStones count={captures} side={capturedSide} skin={skin} />
    </div>
  );
}
