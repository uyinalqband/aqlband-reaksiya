...
const FIRST_TURN_TIME_MS = 120_000;
...
export function CheckersScreen() {
  ...
  return (
    <Screen className="pb-8">
      ...
      <div className="my-3 flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-black/15 px-3 py-2.5">
        <div className="min-w-0">
          <p className="truncate font-display text-sm font-extrabold text-mist-100">
            {statusText}
          </p>
          <p className="mt-0.5 text-[10px] font-bold uppercase tracking-[0.12em] text-mist-600">
            {ratedMatch ? t('v2.ratedMatch') : t('v2.friendMatch')} · {duel.checkers_moves ?? 0} {t('checkers.moves')}
          </p>
        </div>
      ...
      <div className="premium-border mx-auto aspect-square w-full max-w-[29rem] overflow-hidden rounded-[1.7rem] bg-gradient-to-br from-[#293D57] via-[#101B2B] to-[#070D15] p-2 shadow-[0_28px_65px_-28px_rgba(0,0,0,.98)]">
  ...
  return (
    <div
      className={`rounded-2xl border px-3 py-3 transition-colors ${
        active
          ? 'border-emerald-300/45 bg-gradient-to-r from-emerald-500/12 to-ink-800/88 shadow-[0_16px_35px_-28px_rgba(91,229,181,.8)]'
          : 'border-white/10 bg-ink-800/78'
      }`}
    >
...
function CapturedStones({
  ...
  return (
    <div className="mt-3 flex min-h-4 flex-wrap items-center gap-1" aria-label="captured pieces">
      {visible === 0 ? (
        Array.from({ length: 4 }, (_, index) => (
          <span
            key={index}
            className="h-3.5 w-3.5 rounded-full border border-white/5 bg-black/10 opacity-35"
          />
        ))
      ) : (
        Array.from({ length: visible }, (_, index) => (
          <span
            key={index}
            className={`h-4 w-4 rounded-full border shadow-[0_2px_4px_rgba(0,0,0,.45)] ${
              side === 'white'
