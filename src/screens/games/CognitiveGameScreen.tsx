...
import { adaptiveLevel } from '@/features/games/trainingCoach';
...
function makeMathChallenge(level: number, round: number, random: RandomSource): Challenge {
  if (level === 0) {
  ...
  const pressure = Math.max(0, round - 4);
  const divisor = 2 + randomInteger(random, 0, Math.min(10, 7 + pressure));
  const quotient = 3 + randomInteger(random, 0, 11 + pressure * 3);
  const extra = 3 + randomInteger(random, 0, 19 + pressure * 5);

  if (pressure >= 3 && random() > 0.45) {
    const multiplier = 2 + randomInteger(random, 0, Math.min(9, 3 + pressure));
    const answer = quotient + extra * multiplier;
    return numericOptions(
      `${divisor * quotient} ÷ ${divisor} + ${extra} × ${multiplier} = ?`,
      answer,
      10 + pressure * 3,
      random,
    );
  ...
  return numericOptions(
    `${divisor * quotient} ÷ ${divisor} + ${extra} = ?`,
    answer,
    8 + pressure * 3,
    random,
  );
...
function makeOddChallenge(level: number, round: number, random: RandomSource): Challenge {
  const pairs = [
    ['🍎', '🍏'], ['🐶', '🐺'], ['🐼', '🐻'], ['🌕', '🌝'], ['🚗', '🚕'], ['⚽', '🏀'],
    ['😎', '🤓'], ['🦁', '🐯'], ['🍓', '🍒'], ['🚀', '🛸'], ['🎯', '🧿'], ['🐸', '🦎'],
  ] as const;
  const [common, odd] = pickRandom(pairs, random);
  const count = Math.min(36, [6, 9, 12, 16][level] + Math.max(0, round - 4) * 2);
...
export function CognitiveGameScreen() {
  ...
  function beginRound() {
    ...
    if (id === 'odd-one-out') {
      setChallenge(makeOddChallenge(level, round, random));
      setStartedAt(now());
      return;
    }

    if (id === 'mental-math') {
      setChallenge(makeMathChallenge(level, round, random));
      setStartedAt(now());
      return;
    }
    ...
    if (id === 'pattern-memory') {
      ...
      const gridSize = Math.min(36, [9, 12, 16, 20][level] + extra * 5);
      const requested = config.memorySize ?? 4;
      const count = Math.min(
        gridSize - 1,
        requested + level + Math.floor((round - 1) / 2) + extra,
      );
      ...
      schedule(() => {
        ...
        setStartedAt(now());
      }, Math.max(450, [2400, 1750, 1200, 850][level] - extra * 60));
      return;
    }
    ...
    if (id === 'sequence-memory') {
      ...
      const length = Math.min(
        18,
        (config.memorySize ?? 4) + level + Math.floor((round - 1) / 2) + extra,
      );
      ...
      schedule(() => {
        ...
        setStartedAt(now());
      }, Math.max(650, 2700 - level * 500 - extra * 70));
      return;
    }
  ...
  useEffect(() => {
    ...
    // startGame intentionally uses the current render's state setters.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [duelContext, phase, config]);

  useEffect(() => {
    if (duelContext || config || phase !== 'setup' || !id) return;
    const base = getSoloGameSession(id);
    const adaptive = Boolean((location.state as { adaptive?: boolean } | null)?.adaptive);
    startGame(adaptive ? { ...base, difficulty: adaptiveLevel(attempts, id) } : base);
    // startGame intentionally uses the current render's state setters.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [attempts, config, duelContext, id, location.state, phase]);
  ...
  const oddColumns = challenge && challenge.options.length <= 9
    ? 'grid-cols-3'
    : challenge && challenge.options.length > 25
      ? 'grid-cols-6'
      : challenge && challenge.options.length > 16
        ? 'grid-cols-5'
        : 'grid-cols-4';
  const ascendingColumns = challenge && challenge.options.length <= 9 ? 'grid-cols-3' : 'grid-cols-4';
  const patternColumns = patternGridSize === 9
    ? 'grid-cols-3'
    : patternGridSize > 25
      ? 'grid-cols-6'
      : patternGridSize >= 20
  ...
  const activityStat = (() => {
    ...
    return {
      label: t('gameplay.level'),
      value: String(config?.difficulty === 'progressive' ? round : level + 1),
    };
  })();
    ...
    <Screen className="mini-game-screen">
      ...
      {phase === 'playing' && config && (
        <div className="space-y-4">
          ...
          <Card className="mini-game-stage min-h-[390px] overflow-hidden">
                ...
                <div className={`mx-auto grid max-w-sm ${oddColumns} gap-3`}>
                  {challenge.options.map((option, index) => (
                    <button
                      ...
                      className={`aspect-square rounded-2xl border border-ink-600 bg-gradient-to-b from-ink-700 to-ink-800 shadow-card transition-transform active:scale-95 ${
                        challenge.options.length > 25
                          ? 'min-h-12 text-2xl'
                          : challenge.options.length > 16
                            ? 'min-h-14 text-3xl'
                            : 'min-h-20 text-5xl'
                      }`}
                    >
