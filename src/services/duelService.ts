...
import type { DuelGameId, DuelRole, DuelRow, IncomingDuelInvite } from '@/types/duel';
...
export function createDuel(
  ...
  return invokeDuel('duel.invite', {
    ...
    gameConfig: {
      ...
      nBack: config.nBack,
      puzzleShuffle: config.puzzleShuffle,
    },
  });
}
...
export function playTicTacToeMove(duelId: string, cell: number): Promise<DuelResult> {
  return invokeDuel('duel.tic_move', { duelId, cell });
}
...
export function playCheckersMove(
  ...
  return invokeDuel('duel.checkers_move', { duelId, from, to });
}

export function resignCheckers(duelId: string): Promise<DuelResult> {
  return invokeDuel('duel.checkers_resign', { duelId });
}

export function offerCheckersDraw(duelId: string): Promise<DuelResult> {
  return invokeDuel('duel.checkers_offer_draw', { duelId });
}

export function respondCheckersDraw(
  duelId: string,
  accept: boolean,
): Promise<DuelResult> {
  return invokeDuel('duel.checkers_draw_response', { duelId, accept });
}

export function requestCheckersRematch(
  duelId: string,
): Promise<DuelResult> {
  return invokeDuel('duel.checkers_rematch', { duelId });
}

export function sendDuelReaction(
  duelId: string,
  emoji: string,
): Promise<DuelResult> {
  return invokeDuel('duel.react', { duelId, emoji });
}
...
export function subscribeToDuel(
  ...
  const refresh = async () => {
    ...
    try {
      ...
      if (!stopped && result) {
        // Ignore server clock-only responses, but never hide a real duel field
        // change (draw offers, ready state, reactions and other games included).
        const signature = JSON.stringify(result.duel);
