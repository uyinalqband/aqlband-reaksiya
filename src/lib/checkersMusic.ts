...
export const checkersMusic = {
  ...
  move(): void {
    duckMusic(170);
    const offset = (gameStep % 5) * 7;
    playEffectSequence([
      { frequency: 390 + offset, delay: 0, duration: 0.065 },
    ]);
    pulseNoise('effect', 0.045, 0, 0.038, 720);
  },
  ...
  capture(combo = 1): void {
    ...
    duckMusic(360);
    applyEffectsGain();
    pulseNoise('effect', 0.18, 0, 0.16, 520);
    playTone('effect', 145 + Math.min(4, combo) * 18, 0.22, 0, 'sawtooth', 0.09);
    if (combo > 1) {
      playTone('effect', 520 + combo * 65, 0.13, 0.09, 'triangle', 0.08);
    }
  },
