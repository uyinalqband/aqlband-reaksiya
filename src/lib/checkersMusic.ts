...
function musicLevel(): number {
  ...
  const duck = state.ducked ? 0.38 : 1;
  return Math.max(0, Math.min(1, state.musicVolume)) * 0.2 * duck;
}

function effectsLevel(): number {
  return state.effectsEnabled
    ? Math.max(0, Math.min(1, state.effectsVolume)) * 0.42
    : 0;
}
...
/**
 * Short layered transient that resembles a wooden checker touching a wooden
 * board. A tiny deterministic pitch variation avoids a repetitive synthetic
 * click without changing the perceived loudness.
 */
function woodImpact(
  weight: 'light' | 'heavy',
  startDelay = 0,
  ...
  const audio = getContext();
  const destination = effectsGain;
  if (!audio || !destination || !state.effectsEnabled) return;

  woodVariation = (woodVariation + 1) % 7;
  const variation = [0.97, 1.025, 0.99, 1.015, 0.98, 1.03, 1][woodVariation];
  const heavy = weight === 'heavy';
  const start = audio.currentTime + startDelay;
  const duration = heavy ? 0.105 : 0.072;

  const length = Math.max(1, Math.floor(audio.sampleRate * duration));
  const buffer = audio.createBuffer(1, length, audio.sampleRate);
  const data = buffer.getChannelData(0);
  for (let index = 0; index < length; index += 1) {
    const attack = Math.min(1, index / Math.max(1, audio.sampleRate * 0.0015));
    const decay = Math.exp(-index / (length * (heavy ? 0.2 : 0.15)));
  ...
  bodyFilter.frequency.value = (heavy ? 720 : 940) * variation + pitchLift;
  bodyFilter.Q.value = heavy ? 0.8 : 1.05;
  bodyGain.gain.setValueAtTime(heavy ? 0.2 : 0.135, start);
  ...
  // Low wooden body resonance gives the impact substance without a bass boom.
  const resonance = audio.createOscillator();
  const resonanceGain = audio.createGain();
  resonance.type = 'sine';
  resonance.frequency.setValueAtTime(
    (heavy ? 155 : 205) * variation + pitchLift * 0.08,
    start,
  );
  resonance.frequency.exponentialRampToValueAtTime(
    (heavy ? 105 : 145) * variation,
    start + duration,
  );
  resonanceGain.gain.setValueAtTime(0.0001, start);
  resonanceGain.gain.exponentialRampToValueAtTime(heavy ? 0.16 : 0.095, start + 0.003);
  resonanceGain.gain.exponentialRampToValueAtTime(0.0001, start + duration);
  resonance.connect(resonanceGain);
  resonanceGain.connect(destination);
  resonance.start(start);
  resonance.stop(start + duration + 0.02);
}
...
function duckMusic(durationMs = 420): void {
  if (!state.gameActive || !state.musicEnabled) return;
  state.ducked = true;
  applyMusicGain(0.045);
  window.setTimeout(() => {
    state.ducked = false;
    applyMusicGain(0.22);
  }, durationMs);
...
export const checkersMusic = {
  ...
  configure(
    ...
    state.effectsVolume = Math.max(0, Math.min(1, effectsVolume));
  ...
  capture(combo = 1): void {
    ...
    duckMusic(360);
    applyEffectsGain();
    const lift = Math.min(4, Math.max(1, combo)) * 28;
    woodImpact('heavy', 0, lift);
    woodImpact('light', 0.075, lift + 55);
  },
  ...
  promotion(): void {
    ...
    woodImpact('heavy');
    playEffectSequence([
      { frequency: 523.25, delay: 0.09, duration: 0.13 },
      { frequency: 659.25, delay: 0.19, duration: 0.15 },
      { frequency: 783.99, delay: 0.3, duration: 0.2 },
    ]);
  },
  ...
  turn(): void {
    duckMusic(190);
    playEffectSequence([
      { frequency: 540, delay: 0, duration: 0.075 },
      { frequency: 690, delay: 0.07, duration: 0.105 },
    ]);
  },

  invalid(): void {
    playTone('effect', 190, 0.13, 0, 'square', 0.055, -12);
  },

  timeWarning(critical = false): void {
    duckMusic(140);
    playTone(
      'effect',
      critical ? 880 : 660,
  ...
  reaction(emoji: string): void {
    ...
    const root = roots[emoji] ?? 640;
    duckMusic(230);
    playTone('effect', root, 0.09, 0, 'triangle', 0.085);
    playTone('effect', root * 1.25, 0.12, 0.075, 'sine', 0.065);
  },

  friendRequest(): void {
    duckMusic(330);
    playEffectSequence([
      { frequency: 520, delay: 0, duration: 0.1 },
      { frequency: 660, delay: 0.09, duration: 0.12 },
      { frequency: 780, delay: 0.18, duration: 0.16 },
    ]);
  },
};
