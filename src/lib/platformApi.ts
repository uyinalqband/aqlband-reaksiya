...
const API_ERROR_KEYS: Record<string, string> = {
  ...
  proxy_network_error: 'apiErrors.network',
  ...
  game_leaderboard_failed: 'errors.generic',
};
...
export async function invokePlatformApi<T>(
  ...
  const directEndpoint = `${supabaseUrl}/functions/v1/aqlband-api`;
  const useSameOriginProxy =
    typeof window !== 'undefined' &&
    window.location.protocol === 'https:' &&
    window.location.hostname.endsWith('.pages.dev');
  const endpoints = useSameOriginProxy
    ? [`${window.location.origin}/api/platform`, directEndpoint]
    : [directEndpoint];
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      // A missing Pages Function can happen during a rolling deploy. Fall
      // back to the direct endpoint without interrupting the user.
      if (endpoint.endsWith('/api/platform') && candidate.status === 404) {
        continue;
      }
      response = candidate;
      break;
